...
typedef struct {
    ...
    uint8_t ID;  //添加数据包唯一id，nodeID加数据包ID
    char data[40];       // 自定义数据
} DataPacket;
...
//    printf("sniffIndex: %d, forwardID: %02x", sniffIndex, packet->forwardID);
    if (sniffIndex == -1){
        ...
        sniffTable[sniffTableCount].lastSniffTime = HAL_GetTick();
        sniffTable[sniffTableCount].sourceID = packet->forwardID;
        sniffTable[sniffTableCount].snifferID = nodeID;
        sniffTable[sniffTableCount].forwardCount = 0;
        sniffTable[sniffTableCount].sourceCount = 0;
        sniffTable[sniffTableCount].ackCount = 0;
        sniffTable[sniffTableCount].routeReqCount = 0;
        sniffTable[sniffTableCount].routeRepCount = 0;
        sniffTableCount++;
    }
...
// 查询路由
// 查询路由表中是否包含数据包destID的下一个转发ID，若有，则返回路由表条目
...
// 路由查询
// 发送路由查询数据包，查询数据包目的id为FF，查询的目的地址为destID（汇聚节点）
void sendRouteRequest(uint8_t destID) {
    DataPacket packet;
    packet.ID = ++packetID;
...
//        printf("disOLED=%s\r\n", disOLED);
    ...
    {   
        ...
        if (currentMillis - previousMillisA0 >= 100000 || previousMillisA0 == 0) //当前时间刻减去前次执行的时间刻
        {
...
//                                     packet.data);
    ...
    if(currentMillis - previousMillisA1 > 50000 && getRoutReplay == 0){
    sendRouteRequest(targetID);
    previousMillisA1 = HAL_GetTick();
    }
    }
